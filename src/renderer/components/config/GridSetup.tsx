...
class GridSetup extends React.Component<GridSetupProps> {
    ...
    return (
      <div className={classes.root}>
        <AppBar
          ...
          className={clsx(
            classes.appBar,
            this.props.tutorial == SGT.dimensions && classes.backdropTop,
          )}
        >
          <Toolbar className={classes.headerBar}>
            <div className={classes.headerLeft}>
              <Tooltip disableInteractive title="Back" placement="right-end">
                <IconButton
                  ...
                  aria-label="Back"
                  className={clsx(
                    this.props.tutorial == SGT.dimensions && classes.disable,
                  )}
            ...
            <div className={classes.headerRight}>
              <TextField
                ...
                variant="filled"
                className={clsx(
                  this.props.tutorial == SGT.dimensions && classes.highlight,
                )}
              ...
              {this.props.scene.grid.map((row, rowIndex) => (
                <React.Fragment key={rowIndex}>
                  {row.map((cell, colIndex) => {
                    let scene = this.props.allScenes.find(
                      (s) => s.id == cell.sceneID,
                    );
                    let sceneCopy = null;
                    if (cell.sceneCopy && cell.sceneCopy.length > 0) {
                      sceneCopy = this.props.allScenes.find(
                        (s) =>
                          s.id ==
                          this.props.scene.grid[cell.sceneCopy[0]][
                            cell.sceneCopy[1]
                          ].sceneID,
                      );
                    }
                    return (
                      ...
                      >
                        <Button
                          ...
                          style={
                            colors[rowIndex] == undefined ||
                            colors[rowIndex][colIndex] == undefined ||
                            colors[rowIndex][colIndex] == ""
                              ? {}
                              : {
                                  borderStyle: "solid",
                                  borderWidth: 10,
                                  borderColor: colors[rowIndex][colIndex],
                                }
                          }
                          variant="outlined"
                        >
            ...
            >
              {!!this.state.isEditing && (
                <SceneSelect
                  ...
                  value={
                    this.props.scene.grid[this.state.isEditing[0]][
    ...
    for (let row of grid) {
      for (let cell of row) {
        if (
          ...
          (cell.sceneCopy[0] > height - 1 || cell.sceneCopy[1] > width - 1)
        ) {
          cell.sceneCopy = [];
          cell.mirror = false;
        }
  ...
  onDragDrop(
    ...
    if (
      newGrid[rowIndex][colIndex].sceneCopy &&
      newGrid[rowIndex][colIndex].sceneCopy.length > 0
    ) {
      newGrid[newRowIndex][newColIndex].sceneCopy =
        newGrid[rowIndex][colIndex].sceneCopy;
    } else {
