...
import { HTF, TF, VTF } from "../../../common/const";
...
export default class ZoomMove extends React.Component<ZoomMoveProps> {
  ...
  ZoomMoveLayer = (data: { children: React.ReactNode }) => {
    ...
    if (this.props.scene.zoom) {
      if (this.props.scene.zoomRandom) {
        zoomStart =
          (Math.floor(
            Math.random() *
              (this.props.scene.zoomStartMax * 10 -
                this.props.scene.zoomStartMin * 10 +
                1),
          ) +
            this.props.scene.zoomStartMin * 10) /
          10;
        zoomEnd =
          (Math.floor(
            Math.random() *
              (this.props.scene.zoomEndMax * 10 -
                this.props.scene.zoomEndMin * 10 +
                1),
          ) +
            this.props.scene.zoomEndMin * 10) /
          10;
    ...
    switch (this.props.scene.transTF) {
      ...
      case TF.sin:
        const sinRate =
          (Math.abs(this.props.scene.transSinRate - 100) + 2) * 1000;
        transDuration =
          Math.floor(
    ...
    const imageProps = useSpring({
      ...
      config: {
        ...
        easing: getEaseFunction(
          this.props.scene.transEase,
          this.props.scene.transExp,
          this.props.scene.transAmp,
          this.props.scene.transPer,
          this.props.scene.transOv,
        ),
      },
    });
