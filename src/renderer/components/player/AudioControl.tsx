...
class AudioControl extends React.Component<AudioControlProps> {
    ...
    return (
      <React.Fragment key={audio.id}>
        ...
        <Grid
          item
          xs={12}
          className={clsx(!this.props.audioEnabled && classes.noPadding)}
        >
  ...
  componentDidUpdate(props: any, state: any) {
    ...
    if (
      (this.props.audio.tick && !props.audio.tick) ||
      (this.props.audio.tick &&
        props.audio.tickMode == TF.scene &&
        this.props.audio.tickMode != TF.scene)
    ) {
      this.tickLoop(true);
    }
    ...
    if (
      ...
      props.scenePaths !== this.props.scenePaths
    ) {
      ...
      switch (this.props.audio.tickMode) {
        case TF.random:
          timeout =
            Math.floor(
              Math.random() *
                (this.props.audio.tickMaxDelay -
                  this.props.audio.tickMinDelay +
                  1),
            ) + this.props.audio.tickMinDelay;
          break;
        case TF.sin:
          const sinRate =
            (Math.abs(this.props.audio.tickSinRate - 100) + 2) * 1000;
          timeout =
            Math.floor(
