...
class PiwigoDialog extends React.Component<PiwigoDialogProps> {
              ...
              {tags.map((tag: Tag) => {
                ...
                return (
                  <Chip
                    ...
                    onClick={this[
                      isSelected ? "removeSelectedTag" : "addSelectedTag"
                    ].bind(this, tag.id)}
                    label={tag.name}
                  />
                );
              ...
              {sortOrder.map((column) => (
                <Card className={classes.sortCol} key={column.name}>
                  <CardContent
                    ...
                    onClick={this.setColumnDirection.bind(
                      this,
                      column.name,
                  ...
                  >
                    <Checkbox
                      ...
                      onChange={this.toggleSortColumn.bind(
                        this,
                        column.name,
                        !column.enabled,
                      )}
  ...
  removeSelectedAlbum(albumID: number) {
    const { selectedAlbums } = this.state;
    this.setState({
      selectedAlbums: selectedAlbums.filter((t: number) => t !== albumID),
    });
  }
  ...
  removeSelectedTag(tagID: number) {
    const { selectedTags } = this.state;
    this.setState({
      selectedTags: selectedTags.filter((t: number) => t !== tagID),
    });
  }
