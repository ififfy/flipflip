...
class SourceList extends React.Component<SourceListProps> {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <AutoSizer>
          {({ height, width }: { height: number; width: number }) => (
            <List
              ...
              disablePadding
              className={clsx(
                (this.props.tutorial == SDT.source ||
        ...
        >
          {!!this.state.clipMenu &&
            this.state.clipMenu.clips.map((c, index) => (
              <MenuItem key={c.id}>
                <ListItemText
                  primary={
                    "(" +
                ...
                <ListItemSecondaryAction>
                  <Switch
                    size="small"
                    checked={
                      !this.state.clipMenu.disabledClips ||
                      !this.state.clipMenu.disabledClips.includes(c.id)
                    }
                    onChange={this.onToggleClip.bind(
                      this,
                      this.state.clipMenu,
                      c,
                    )}
        ...
        >
          ...
          <DialogActions>
            <Button
              onClick={this.onCloseBlacklist.bind(this)}
              color="secondary"
            >
              Cancel
            </Button>
              ...
              <FormControlLabel
                control={
                  <Tooltip
                    disableInteractive
                    title={
                      "Enable this to treat directories directly inside this one as their own individual sources"
                    }
                  >
                    <Switch
                      checked={this.state.sourceOptions.dirOfSources}
          ...
          >
            <DialogContent>
              ...
              <TextField
                ...
                margin="dense"
                value={
                  this.state.sourceOptions.subtitleFile == null
                    ? ""
                    : this.state.sourceOptions.subtitleFile
                }
                InputProps={{
                  endAdornment: (
        ...
        {this.state.deleteDialog != null && (
          ...
          >
            ...
            <DialogActions>
              <Button
                onClick={this.onCloseDeleteDialog.bind(this)}
                color="secondary"
              >
                Cancel
              </Button>
  ...
  componentDidUpdate(props: any) {
    if (
      this.props.sources.length != props.sources.length &&
  ...
  onFinishDelete() {
    window.ipc.deleteSource(this.state.deleteDialog.url).then(() => {
  ...
  onRemove(source: LibrarySource) {
    if (this.props.onUpdateScene) {
      this.props.onUpdateScene(
        (s) => (s.sources = s.sources.filter((s) => s.id != source.id)),
      );
    } else if (this.props.onUpdateLibrary) {
      this.props.onUpdateSelected(
        this.props.selected.filter((url) => url != source.url),
      );
      this.props.onUpdateLibrary((l) => {
        l.forEach((s, index) => {
          if (s.id == source.id) {
  ...
  onToggleSelect(e: MouseEvent) {
    ...
    if (newSelected.includes(source)) {
      ...
      if (
        this.props.sources.map((s) => s.url).includes(this._lastChecked) &&
        this._shiftDown
      ) {
        let start = false;
  ...
  changeWeight(intString: number) {
    this.props.onUpdateScene((scene) => {
      const source = scene.sources.find(
        (ls) => ls.id == this.state.weightMenu.id,
      );
  ...
  onToggleClip(s: LibrarySource, c: Clip) {
    this.props.onUpdateScene((scene) => {
      ...
      if (!source.disabledClips) {
        ...
        source.disabledClips = source.disabledClips.filter(
          (d: number) => d != c.id,
        );
      } else {
  ...
  onEndEdit(newURL: string) {
    if (this.props.onUpdateScene) {
      this.props.onUpdateScene((s) => {
        ...
        editSource.tags =
          librarySource && librarySource.tags
            ? librarySource.tags
            : editSource.tags;
        editSource.clips =
          librarySource && librarySource.clips
            ? librarySource.clips
            : editSource.clips;
        editSource.blacklist =
          librarySource && librarySource.blacklist
            ? librarySource.blacklist
            : editSource.blacklist;
        editSource.count = librarySource
          ? librarySource.count
          : sourceChanged
            ? 0
            : editSource.count;
        editSource.countComplete = librarySource
          ? librarySource.countComplete
          : sourceChanged
            ? false
            : editSource.countComplete;
        editSource.duration = librarySource
          ? librarySource.duration
          : sourceChanged
            ? null
            : editSource.duration;
        editSource.resolution = librarySource
      ...
      this.props.onUpdateLibrary((l) => {
        ...
        editSource.countComplete = sourceChanged
          ? false
          : editSource.countComplete;
  ...
  onClean(source: LibrarySource) {
    window.ipc.getCachePath(source.url, this.props.config).then((cachePath) => {
  ...
  onFinishBlacklist() {
    this.props.onEditBlacklist(
      this.state.blacklistSource,
      this.state.editBlacklist,
    );
  ...
  onChangeBlacklist(e: MouseEvent) {
    this.setState({
      editBlacklist: (e.currentTarget as HTMLInputElement).value,
    });
  }

  onEditBlacklist(source: LibrarySource) {
    this.setState({
      blacklistSource: source.url,
      editBlacklist: source.blacklist.join("\n"),
    });
  }
  ...
  onSourceOptions(source: LibrarySource) {
    this.setState({
      ...
      sourceOptionsType: getSourceType(source.url),
    });
  }
    ...
    if (this.props.onUpdateScene) {
      this.props.onUpdateScene((s) => {
        const source = s.sources.find(
          (s) => s.url == this.state.sourceOptions.url,
        );
  ...
  VirtualList(props: any) {
    ...
    return (
      <FixedSizeList
        ...
        itemData={
          this.props.tutorial ? [this.props.sources[0]] : this.props.sources
        }
  ...
  Row(props: any) {
    const { index } = props;
    return <this.SortableItem index={index} value={props} />;
  }
}
